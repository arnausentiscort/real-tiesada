...
function TimelineChart({ match, matchStats }) {
  ...
  // Pauses
  const pauses = subs.filter(s=>s.onPitch?.length===0).map((s,i) => {
    const next = subs[subs.indexOf(s)+1];
    return next ? {start:parseTime(s.time), end:parseTime(next.time)} : null;
  }).filter(Boolean);
      ...
      <div className="bg-[#1E1E1E] rounded-2xl border border-white/5 p-5 overflow-x-auto">
        ...
        <div style={{minWidth:560}}>
          ...
          {allPlayers.map(name => {
            ...
            const ps    = playerStats.find(p=>p.player===name);
            const dev   = ps ? ps.deviation : (total - idealSecs);
                ...
                <div className="flex-1 relative" style={{height:22}}>
                  ...
                  {campS.map((s,i) => (
                    <div key={`c${i}`} className="absolute rounded border border-[#E5C07B]/10 hover:border-[#E5C07B]/40 transition-all"
                      ...
                      title={`${shirtName(name)}: ${formatTime(s.start)} → ${formatTime(s.end)}`}/>
                  ))}
                  {gkS.map((s,i) => (
                    <div key={`g${i}`} className="absolute rounded border border-emerald-500/20 hover:border-emerald-500/50 transition-all"
                      ...
                      title={`${shirtName(name)} (porter): ${formatTime(s.start)} → ${formatTime(s.end)}`}/>
                  ))}
                  {goals.map((g,i) => (
